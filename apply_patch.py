...
def extract_part_number(patch_filename):
    """Extract part number from patch filename like 'consolidated_verified_notes_v2_8_part_002_patch.json'"""
...
def main():
    # Find all patch files in golden_patches directory
    golden_patches_dir = Path("golden_patches")
    if not golden_patches_dir.exists():
        print(f"Error: Directory {golden_patches_dir} not found")
        return

    patch_files = sorted(golden_patches_dir.glob("consolidated_verified_notes_v2_8_part_*_patch.json"))
    
    if not patch_files:
        print("No patch files found matching pattern 'consolidated_verified_notes_v2_8_part_*_patch.json'")
        return
    ...
    for patch_file in patch_files:
        ...
        original_path = f"unpatched_goldens/consolidated_verified_notes_v2_8_part_{part_num}.json"
        patch_path = str(patch_file)
        output_path = f"golden_extractions/consolidated_verified_notes_v2_8_part_{part_num}.json"
